...
case "$1" in
  start)
    ...
    do_start
    touch /var/lock/subsys/logstash-{{ logstash_version }}
    set_pidfile
    ;;
  stop)
    echo -n "Stopping $DESC: "
    do_stop
    rm /var/lock/subsys/logstash-{{ logstash_version }}
    rm $PIDFILE
    ;;
  restart|reload)
